...
psql --command 'COPY public.people_db_person (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-people-2024-08-15.csv

echo "Loading people_db_race-2024-08-15.csv to database"
psql --command 'COPY public.people_db_race (id, race) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people_db_race-2024-08-15.csv
...
psql --command 'COPY public.people_db_school (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-schools-2024-08-15.csv
...
psql --command 'COPY public.search_court (
	       ...
	       ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/courts-2024-08-15.csv
...
psql --command 'COPY public.people_db_position (
	       ...
	       supervisor_id
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-positions-2024-08-15.csv
...
psql --command 'COPY public.recap_fjcintegrateddatabase (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/fjc-integrated-database-2024-08-15.csv
...
psql --command 'COPY public.search_originatingcourtinformation (
	       ...
	       ordering_judge_id
	       ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/originating-court-information-2024-08-15.csv
...
psql --command 'COPY public.search_docket (id, date_created, date_modified, source, appeal_from_str,
	       ...
	       ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/dockets-2024-08-15.csv
...
psql --command 'COPY public.search_opinioncluster (
   ...
   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/opinion-clusters-2024-08-15.csv
...
psql --command 'COPY public.search_opinioncluster_panel (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/search_opinioncluster_panel-2024-08-15.csv
...
psql --command 'COPY public.search_opinioncluster_non_participating_judges (
	       id, opinioncluster_id, person_id
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/search_opinioncluster_non_participating_judges-2024-08-15.csv
...
psql --command 'COPY public.search_opinion (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/opinions-2024-08-15.csv
...
) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/search_opinion_joined_by-2024-08-15.csv
...
state, zip_code, country_code, court_id) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/courthouses-2024-08-15.csv

echo "Loading court-appeals-to-2024-08-15.csv to database"
psql --command 'COPY public.search_court_appeals_to (id, from_court_id, to_court_id) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/court-appeals-to-2024-08-15.csv
...
psql --command 'COPY public.search_opinionscited (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/citation-map-2024-08-15.csv
...
psql --command 'COPY public.search_citation (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/citations-2024-08-15.csv
...
psql --command 'COPY public.search_parenthetical (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/parentheticals-2024-08-15.csv
...
psql --command 'COPY public.audio_audio (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/oral-arguments-2024-08-15.csv
...
psql --command 'COPY public.people_db_retentionevent (
	       ...
	       won, position_id
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-retention-events-2024-08-15.csv
...
psql --command 'COPY public.people_db_education (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-educations-2024-08-15.csv
...
psql --command 'COPY public.people_db_politicalaffiliation (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-political-affiliations-2024-08-15.csv
...
psql --command 'COPY public.people_db_person_race (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people-db-races-2024-08-15.csv
...
psql --command 'COPY public.disclosures_financialdisclosure (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-2024-08-15.csv
...
psql --command 'COPY public.disclosures_investment (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosure-investments-2024-08-15.csv
...
psql --command 'COPY public.disclosures_position (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-positions-2024-08-15.csv
...
psql --command 'COPY public.disclosures_agreement (
	       ...
	       redacted, financial_disclosure_id
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-agreements-2024-08-15.csv
...
psql --command 'COPY public.disclosures_noninvestmentincome (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-non-investment-income-2024-08-15.csv
...
psql --command 'COPY public.disclosures_spouseincome (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-spousal-income-2024-08-15.csv
...
psql --command 'COPY public.disclosures_reimbursement (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-reimbursements-2024-08-15.csv
...
psql --command 'COPY public.disclosures_gift (
	       ...
	       redacted, financial_disclosure_id
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-gifts-2024-08-15.csv
...
psql --command 'COPY public.disclosures_debt (
	   ...
	   ) FROM STDIN WITH (FORMAT csv, ENCODING utf8, HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/financial-disclosures-debts-2024-08-15.csv
