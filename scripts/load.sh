...
psql --command 'COPY public.people_db_race (id, race) FROM STDIN WITH (FORMAT csv, ENCODING utf8, QUOTE "`", HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/people_db_race-2024-08-15.csv
...
psql --command 'COPY public.search_court_appeals_to (id, from_court_id, to_court_id) FROM STDIN WITH (FORMAT csv, ENCODING utf8, QUOTE "`", HEADER)' --host "$BULK_DB_HOST" --username "$BULK_DB_USER" --dbname "$BULK_DB_NAME" < $BULK_DIR/court-appeals-to-2024-08-15.csv
